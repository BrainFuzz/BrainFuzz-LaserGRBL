...
namespace LaserGRBL {
  public class GrblFile :IEnumerable<GrblCommand> {
	...
	private List<GrblCommand> _commandList = new List<GrblCommand>();
	private ProgramRange _range = new ProgramRange();
	...
	public GrblFile(decimal x, decimal y, decimal x1, decimal y1) {
	  _range.UpdateXYRange(new GrblCommand.Element('X', x), new GrblCommand.Element('Y', y), false);
	  _range.UpdateXYRange(new GrblCommand.Element('X', x1), new GrblCommand.Element('Y', y1), false);
	}
	...
	public void SaveProgram(string filename, bool header, bool footer, bool between, int cycles) {
	  try {
		using (System.IO.StreamWriter streamWriter = new System.IO.StreamWriter(filename)) {
		  if (header)
			streamWriter.WriteLine(Settings.GetObject("GCode.CustomHeader", GrblCore.GCODE_STD_HEADER));

		  for (int i = 0; i < cycles; i++) {
			foreach (GrblCommand cmd in _commandList)
			  streamWriter.WriteLine(cmd.Command);


			if (between && i < cycles - 1)
			  streamWriter.WriteLine(Settings.GetObject("GCode.CustomPasses", GrblCore.GCODE_STD_PASSES));
		  }

		  if (footer)
			streamWriter.WriteLine(Settings.GetObject("GCode.CustomFooter", GrblCore.GCODE_STD_FOOTER));
	...
	public void LoadFile(string filename, bool appendToFile) {
		...
		using (System.IO.StreamReader streamReader = new System.IO.StreamReader(filename)) {
		  ...
		  while ((currentLine = streamReader.ReadLine()) != null)
			if ((currentLine = currentLine.Trim()).Length > 0) {
			  GrblCommand cmd = new GrblCommand(currentLine);
			  if (!cmd.IsEmpty)
	...
	private abstract class ColorSegment {
	  ...
	  public ColorSegment(int colour, int len, bool rev) {
		mColor = colour;
		mPixLen = rev ? -len : len;
	  }
	  ...
	  public bool Fast(L2LConf c) { return c.pwm ? mColor == 0 : mColor <= 125; }
	...
	public static bool VectorFilling(RasterConverter.ImageProcessor.Direction direction) {
	  return direction == RasterConverter.ImageProcessor.Direction.NewDiagonal ||
	  direction == RasterConverter.ImageProcessor.Direction.NewHorizontal ||
	  direction == RasterConverter.ImageProcessor.Direction.NewVertical ||
	  direction == RasterConverter.ImageProcessor.Direction.NewReverseDiagonal ||
	  direction == RasterConverter.ImageProcessor.Direction.NewGrid ||
	  direction == RasterConverter.ImageProcessor.Direction.NewDiagonalGrid ||
	  direction == RasterConverter.ImageProcessor.Direction.NewCross ||
	  direction == RasterConverter.ImageProcessor.Direction.NewDiagonalCross ||
	  direction == RasterConverter.ImageProcessor.Direction.NewSquares ||
	  direction == RasterConverter.ImageProcessor.Direction.NewZigZag ||
	  direction == RasterConverter.ImageProcessor.Direction.NewHilbert ||
	  direction == RasterConverter.ImageProcessor.Direction.NewInsetFilling;
	}

	public static bool TimeConsumingFilling(RasterConverter.ImageProcessor.Direction direction) {
	  return
	  direction == RasterConverter.ImageProcessor.Direction.NewCross ||
	  direction == RasterConverter.ImageProcessor.Direction.NewDiagonalCross ||
	  direction == RasterConverter.ImageProcessor.Direction.NewSquares;
	...
	public void LoadImagePotrace(Bitmap bmp, string filename, bool UseSpotRemoval, int SpotRemoval, bool UseSmoothing, decimal Smoothing, bool UseOptimize, decimal Optimize, bool useOptimizeFast, L2LConf c, bool append, GrblCore core) {
		...
		using (Bitmap ptb = new Bitmap(bmp.Width, bmp.Height)) {
		  using (Graphics g = Graphics.FromImage(ptb)) {
			...
			using (Bitmap resampled = RasterConverter.ImageTransform.ResizeImage(ptb, new Size((int) (bmp.Width * c.fres / c.res) + 1, (int) (bmp.Height * c.fres / c.res) + 1), true, InterpolationMode.HighQualityBicubic)) {
			  if (c.pwm)
				_commandList.Add(new GrblCommand(String.Format("{0} S0", c.lOn))); //laser on and power to zero
			  else
				_commandList.Add(new GrblCommand(String.Format("{0} S255", c.lOff))); //laser off and power to max power

			  //set speed to markspeed
			  // For marlin, need to specify G1 each time :
			  // list.Add(new GrblCommand(String.Format("G1 F{0}", c.markSpeed)));
			  _commandList.Add(new GrblCommand(String.Format("F{0}", c.markSpeed)));

			  c.vectorfilling = true;
			  ImageLine2Line(resampled, c);

			  //laser off
			  _commandList.Add(new GrblCommand(c.lOff));
			}
		  }
		}
	  }
	  ...
	  if (supportPWM)
		_commandList.Add(new GrblCommand($"{c.lOn} S0"));   //laser on and power to 0
	  else
		_commandList.Add(new GrblCommand($"{c.lOff} S{c.maxPower}"));   //laser off and power to maxPower
	  ...
	  if (flist != null) {
		...
		foreach (string code in gc)
		  _commandList.Add(new GrblCommand(code));
	  }
	  ...
	  if (plist != null) //always true
	  {
		...
		_commandList.Add(new GrblCommand(String.Format("F{0}", c.borderSpeed)));
		foreach (string code in gc)
		  _commandList.Add(new GrblCommand(code));
	  }
	  ...
	  //laser off (superflua??)
	  if (supportPWM)
		_commandList.Add(new GrblCommand(c.lOff));  //necessaria perché finisce con solo S0
	...
	public void LoadImageL2L(Bitmap bmp, string filename, L2LConf c, bool append) {
	  ...
	  _commandList.Add(new GrblCommand(String.Format("{0} X{1} Y{2} F{3}", skipcmd, formatnumber(c.oX), formatnumber(c.oY), c.markSpeed)));
	  if (c.pwm)
		_commandList.Add(new GrblCommand(String.Format("{0} S0", c.lOn))); //laser on and power to zero
	  else
		_commandList.Add(new GrblCommand(String.Format("{0} S255", c.lOff))); //laser off and power to maxpower
	...
	private List<ColorSegment> GetSegments(Bitmap bmp, L2LConf c) {
	  bool isUnidirectional = Settings.GetObject("Unidirectional Engraving", false);
	  ...
	  if (c.dir == RasterConverter.ImageProcessor.Direction.Horizontal || c.dir == RasterConverter.ImageProcessor.Direction.Vertical) {
		bool isHorizontal = (c.dir == RasterConverter.ImageProcessor.Direction.Horizontal); //horizontal/vertical

		for (int i = 0; i < (isHorizontal ? bmp.Height : bmp.Width); i++) {
		  bool d = isUnidirectional || IsEven(i); //direct/reverse
		  int prevCol = -1;
		  int len = -1;

		  for (int j = d ? 0 : (isHorizontal ? bmp.Width - 1 : bmp.Height - 1); d ? (j < (isHorizontal ? bmp.Width : bmp.Height)) : (j >= 0); j = (d ? j + 1 : j - 1))
			ExtractSegment(bmp, isHorizontal ? j : i, isHorizontal ? i : j, !d, ref len, ref prevCol, rv, c); //extract different segments
		  ...
		  if (isUnidirectional) // add "go back"
		  {
			if (isHorizontal)
		...
		*/
		...
		for (int slice = 0; slice < width + height - 1; ++slice) {
		  bool direction = isUnidirectional || IsEven(slice); //direct/reverse
		  ...
		  for (int j = (direction ? z1 : slice - z2); direction ? j <= slice - z2 : j >= z1; j = (direction ? j + 1 : j - 1))
			ExtractSegment(bmp, j, slice - j, !direction, ref len, ref prevCol, rv, c); //extract different segments
		  rv.Add(new DSegment(prevCol, len + 1, !direction)); //close last segment
		  ...
		  if (slice < Math.Min(width, height) - 1) //first part of the image
		  {
			if (direction && !isUnidirectional)
			  rv.Add(new HSeparator()); //new line
			else
			  rv.Add(new VSeparator()); //new line
		  } else if (slice >= Math.Max(width, height) - 1) //third part of image
			{
			if (direction && !isUnidirectional)
			  rv.Add(new VSeparator()); //new line
			else
	...
	private static double SquareDistance(dPoint p1, dPoint p2) {
	  double dX = p2.X - p1.X;
	  double dY = p2.Y - p1.Y;
	  return ((dX * dX) + (dY * dY));
	}

	private static double SquareDistanceZero(dPoint point) {
	...
	private static bool IsEven(int value) { return value % 2 == 0; }

	public int Count { get { return _commandList.Count; } }

	public TimeSpan EstimatedTime { get { return _elapsedTime; } }
	...
	public CartesianQuadrant Quadrant {
	  get {
		if (!_range.DrawingRange.ValidRange)
		  return CartesianQuadrant.Unknown;
		else if (_range.DrawingRange.X.Min >= 0 && _range.DrawingRange.Y.Min >= 0)
		  return CartesianQuadrant.I;
		else if (_range.DrawingRange.X.Max <= 0 && _range.DrawingRange.Y.Min >= 0)
		  return CartesianQuadrant.II;
		else if (_range.DrawingRange.X.Max <= 0 && _range.DrawingRange.Y.Max <= 0)
		  return CartesianQuadrant.III;
		else if (_range.DrawingRange.X.Min >= 0 && _range.DrawingRange.Y.Max <= 0)
	...
	internal void DrawOnGraphics(Graphics g, Size size) {
	  if (!_range.MovingRange.ValidRange)
		return;

	  GrblCommand.StatePositionBuilder spb = new GrblCommand.StatePositionBuilder();
	  ProgramRange.XYRange scaleRange = _range.MovingRange;
	...
	private void DrawJobPreview(Graphics graphics, GrblCommand.StatePositionBuilder spb, float zoom) {
	  ...
	  foreach (GrblCommand cmd in _commandList) {
		try {
		  ...
		  if (spb.TrueMovement()) {
			Color linecolor = Color.FromArgb(spb.GetCurrentAlpha(_range.SpindleRange), firstline ? ColorScheme.PreviewFirstMovement : spb.LaserBurning ? ColorScheme.PreviewLaserPower : ColorScheme.PreviewOtherMovement);
			using (Pen pen = GetPen(linecolor)) {
			  ...
			  if (spb.G0G1 && cmd.IsLinearMovement && pen.Color.A > 0) {
				graphics.DrawLine(pen, new PointF((float) spb.X.Previous, (float) spb.Y.Previous), new PointF((float) spb.X.Number, (float) spb.Y.Number));
			  } else if (spb.G2G3 && cmd.IsArcMovement && pen.Color.A > 0) {
				GrblCommand.G2G3Helper ah = spb.GetArcHelper(cmd);

				if (ah.RectW > 0 && ah.RectH > 0) {
				  try { graphics.DrawArc(pen, (float) ah.RectX, (float) ah.RectY, (float) ah.RectW, (float) ah.RectH, (float) (ah.StartAngle * 180 / Math.PI), (float) (ah.AngularWidth * 180 / Math.PI)); } catch { System.Diagnostics.Debug.WriteLine(String.Format("Ex drwing arc: W{0} H{1}", ah.RectW, ah.RectH)); }
				}
			  }
	...
	private void Analyze() //analyze the file and build global range and timing for each command
	{
	  ...
	  _range.ResetRange();
	  _range.UpdateXYRange("X0", "Y0", false);
	  ...
	  foreach (GrblCommand cmd in _commandList) {
		try {
		  ...
		  _range.UpdateSRange(spb.S);

		  if (spb.LastArcHelperResult != null)
			_range.UpdateXYRange(spb.LastArcHelperResult.BBox.X, spb.LastArcHelperResult.BBox.Y, spb.LastArcHelperResult.BBox.Width, spb.LastArcHelperResult.BBox.Height, spb.LaserBurning);
		  else
			_range.UpdateXYRange(spb.X, spb.Y, spb.LaserBurning);

		  _elapsedTime += delay;
		  cmd.SetOffset(_elapsedTime);
		} catch (Exception ex) { throw ex; } finally { cmd.DeleteHelper(); }
	  }
	}
	...
	private void DrawJobRange(Graphics g, Size s, float zoom) {
	  ...
	  if (_range.DrawingRange.ValidRange) {
		using (Pen pen = GetPen(ColorScheme.PreviewJobRange)) {
		  ...
		  g.DrawLine(pen, -wSize.Width, (float) _range.DrawingRange.Y.Min, wSize.Width, (float) _range.DrawingRange.Y.Min);
		  g.DrawLine(pen, -wSize.Width, (float) _range.DrawingRange.Y.Max, wSize.Width, (float) _range.DrawingRange.Y.Max);
		  g.DrawLine(pen, (float) _range.DrawingRange.X.Min, -wSize.Height, (float) _range.DrawingRange.X.Min, wSize.Height);
		  g.DrawLine(pen, (float) _range.DrawingRange.X.Max, -wSize.Height, (float) _range.DrawingRange.X.Max, wSize.Height);
		  ...
		  string format = "0";
		  if (_range.DrawingRange.Width < 50 && _range.DrawingRange.Height < 50)
			format = "0.0";

		  DrawString(g, zoom, 0, _range.DrawingRange.Y.Min, _range.DrawingRange.Y.Min.ToString(format), false, true, !right, false, ColorScheme.PreviewText);
		  DrawString(g, zoom, 0, _range.DrawingRange.Y.Max, _range.DrawingRange.Y.Max.ToString(format), false, true, !right, false, ColorScheme.PreviewText);
		  DrawString(g, zoom, _range.DrawingRange.X.Min, 0, _range.DrawingRange.X.Min.ToString(format), true, false, false, top, ColorScheme.PreviewText);
		  DrawString(g, zoom, _range.DrawingRange.X.Max, 0, _range.DrawingRange.X.Max.ToString(format), true, false, false, top, ColorScheme.PreviewText);
		}
	  }
	  ...
	  using (Pen pen = GetPen(ColorScheme.PreviewRuler)) {
		...
		if (_range.DrawingRange.ValidRange && _range.DrawingRange.Width < 50 && _range.DrawingRange.Height < 50)
		  format = "0.0";
	...
	private static Brush GetBrush(Color color) { return new SolidBrush(color); }
	...
	private static void DrawString(Graphics g, float zoom, decimal curX, decimal curY, string text, bool centerX, bool centerY, bool subtractX, bool subtractY, Color color, float rotation = 0) {
	  ...
	  using (Font f = new Font(FontFamily.GenericMonospace, 8 * 1 / zoom)) {
		...
		using (Brush b = GetBrush(color)) { DrawRotatedTextAt(g, rotation, text, f, b, (float) curX - offsetX, (float) -curY - offsetY); }
	...
	System.Collections.Generic.IEnumerator<GrblCommand> IEnumerable<GrblCommand>.GetEnumerator() { return _commandList.GetEnumerator(); }


	public System.Collections.IEnumerator GetEnumerator() { return _commandList.GetEnumerator(); }

	public ProgramRange Range { get { return _range; } }

	public GrblCommand this[int index] { get { return _commandList[index]; } }
  ...
  public class ProgramRange {
	public class XYRange {
	  public class Range {
		...
		public bool ValidRange { get { return Min != decimal.MaxValue && Max != decimal.MinValue; } }
	  }
	  ...
	  public bool ValidRange { get { return X.ValidRange && Y.ValidRange; } }

	  public decimal Width { get { return X.Max - X.Min; } }

	  public decimal Height { get { return Y.Max - Y.Min; } }
	...
	public class SRange {
	  public class Range {
		...
		public bool ValidRange { get { return Min != Max && Min != decimal.MaxValue && Max != decimal.MinValue && Max > 0; } }
	  }
	  ...
	  public bool ValidRange { get { return S.ValidRange; } }
	}
	...
	public void UpdateSRange(GrblCommand.Element S) { if (S != null) SpindleRange.UpdateRange(S.Number); }
