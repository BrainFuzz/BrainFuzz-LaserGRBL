...
namespace LaserGRBL {
  ...
  public partial class ConnectLogForm :System.Windows.Forms.UserControl {
	...
	public ComWrapper.WrapperType comWrapper;

	GrblCore _grblCore;
	private string _loadedFileName;

	public ConnectLogForm() {
	  comWrapper = Settings.GetObject("ComWrapper Protocol", ComWrapper.WrapperType.UsbSerial);
	  InitializeComponent();
	}
	...
	public void SetCore(GrblCore core) {
	  ...
	  _grblCore.OnLoopCountChange += OnLoopCountChanged;

	  CmdLog.SetCom(core);

	  PB.Bars.Add(new UserControls.DoubleProgressBar.Bar(Color.LightSkyBlue));
	  PB.Bars.Add(new UserControls.DoubleProgressBar.Bar(Color.Pink));
	...
	private void RestoreConf() {
	  ...
	  if (comWrapper == ComWrapper.WrapperType.Telnet)
		TxtAddress.Text = Settings.GetObject("Telnet Address", "127.0.0.1:23");
	  else if (comWrapper == ComWrapper.WrapperType.LaserWebESP8266)
	...
	private void InitSpeedCB() {
	  CBSpeed.BeginUpdate();
	  CBSpeed.Items.AddRange(baudRates);
	  CBSpeed.EndUpdate();
	}

	private void InitPortCB() 	{
	  string currentPort = CBPort.SelectedItem as string;
	  CBPort.BeginUpdate();
	  CBPort.Items.Clear();

	  foreach (string portId in System.IO.Ports.SerialPort.GetPortNames()) {
		string purgename = portId;
	...
	void BtnConnectDisconnectClick(object sender, EventArgs e) {
	  if (_grblCore.MachineStatus == GrblCore.MacStatus.Disconnected)
		_grblCore.OpenCom();
	  else if (!(_grblCore.InProgram && MessageBox.Show(Strings.DisconnectAnyway, Strings.WarnMessageBoxHeader, MessageBoxButtons.YesNo, 
	...
	void ApplyConfig() {
	  if ((comWrapper == ComWrapper.WrapperType.UsbSerial || comWrapper == ComWrapper.WrapperType.UsbSerial2) && CBPort.Text != null && CBSpeed.SelectedItem != null)
		_grblCore.Configure(comWrapper, CBPort.Text, (int) CBSpeed.SelectedItem);
	  else if (comWrapper == ComWrapper.WrapperType.Telnet || comWrapper == ComWrapper.WrapperType.LaserWebESP8266)
		_grblCore.Configure(comWrapper, (string) TxtAddress.Text);
	  else if (comWrapper == ComWrapper.WrapperType.Emulator)
		_grblCore.Configure(comWrapper);
	}

	void BtnOpenClick(object sender, EventArgs e) {
	  _grblCore.OpenFile(ParentForm);
	}

	void BtnRunProgramClick(object sender, EventArgs e) {
	  _grblCore.RunProgram(ParentForm);
	}
	void TxtManualCommandCommandEntered(string command) {
	  _grblCore.EnqueueCommand(new GrblCommand(command, 0, true));
	}

	public void TimerUpdate() {
	  SuspendLayout();

	  if (!_grblCore.IsConnected && System.IO.Ports.SerialPort.GetPortNames().Length != CBPort.Items.Count)
		InitPortCB();

	  PB.Maximum = _grblCore.ProgramTarget;
	  PB.Bars[0].Value = _grblCore.ProgramSent;
	  PB.Bars[1].Value = _grblCore.ProgramExecuted;

	  string val = Tools.Utils.TimeSpanToString(_grblCore.ProgramTime, Tools.Utils.TimePrecision.Minute, Tools.Utils.TimePrecision.Second, " ,", true);

	  if (val != "now")
		PB.PercString = val;
	  else if (_grblCore.InProgram)
		PB.PercString = "0 sec";
	  else
		PB.PercString = "";
	  ...
	  */
	  ...
	  TT.SetToolTip(BtnConnectDisconnect, _grblCore.IsConnected ? Strings.BtnDisconnectTT : Strings.BtnConnectTT);

	  BtnConnectDisconnect.UseAltImage = _grblCore.IsConnected;
	  BtnRunProgram.Enabled = _grblCore.CanSendFile;
	  BtnRunProgram.Visible = !_grblCore.CanAbortProgram;
	  BtnAbortProgram.Visible = _grblCore.CanAbortProgram;
	  BtnOpen.Enabled = _grblCore.CanLoadNewFile;

	  bool old = TxtManualCommand.Enabled;
	  TxtManualCommand.Enabled = _grblCore.CanSendManualCommand;
	  //if (old == false && TxtManualCommand.Enabled == true)
	  //	TxtManualCommand.Focus();

	  //CBProtocol.Enabled = !Core.IsOpen;
	  CBPort.Enabled = !_grblCore.IsConnected;
	  CBSpeed.Enabled = !_grblCore.IsConnected;
	  TxtAddress.Enabled = !_grblCore.IsConnected;

	  CmdLog.TimerUpdate();

	  if (!_grblCore.IsConnected) {
		ComWrapper.WrapperType actualWrapper = Settings.GetObject("ComWrapper Protocol", ComWrapper.WrapperType.UsbSerial);
		if (actualWrapper != comWrapper) {
		  comWrapper = actualWrapper;
		  UpdateConf();
		}
	  }
	...
	private void UpdateConf() {
	  ...
	  CBPort.Visible = CBSpeed.Visible = LblComPort.Visible = LblBaudRate.Visible = (comWrapper == ComWrapper.WrapperType.UsbSerial || comWrapper == ComWrapper.WrapperType.UsbSerial2);
	  TxtAddress.Visible = LblAddress.Visible = (comWrapper == ComWrapper.WrapperType.Telnet || comWrapper == ComWrapper.WrapperType.LaserWebESP8266);
	  LblAddress.Text = (comWrapper == ComWrapper.WrapperType.Telnet ? "IP:PORT" : "Socket URL");
	  TxtEmulator.Visible = LblEmulator.Visible = (comWrapper == ComWrapper.WrapperType.Emulator);
	...
	private void UDLoopCounter_ValueChanged(object sender, EventArgs e) {
	  _grblCore.LoopCount = UDLoopCounter.Value;
	}
	...
	internal void ConfigFromDiscovery(string config) {
	  if (TxtAddress.Visible && TxtAddress.Enabled && config != null) {
		...
		if (BtnConnectDisconnect.Enabled && _grblCore.MachineStatus == GrblCore.MacStatus.Disconnected)
