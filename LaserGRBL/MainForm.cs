...
namespace LaserGRBL {
  public partial class MainForm :Form {
	...
	private UsageStats.MessageData _toolBarMessage;
	...
	public MainForm() {
	  ...
	  if (ftype == Firmware.Smoothie)
		_grblCore = new SmoothieCore(this, PreviewForm, JogForm);
	  else if (ftype == Firmware.Marlin)
		_grblCore = new MarlinCore(this, PreviewForm, JogForm);
	  else if (ftype == Firmware.VigoWork)
		_grblCore = new VigoCore(this, PreviewForm, JogForm);
	  else
		_grblCore = new GrblCore(this, PreviewForm, JogForm);
	  ExceptionManager.grblCore = _grblCore;

	  if (true) //use multi instance trigger
	  {
		SincroStart.StartListen(_grblCore);
		MultipleInstanceTimer.Enabled = true;
	  }

	  MnGrblConfig.Visible = _grblCore.UIShowGrblConfig;
	  MnUnlock.Visible = _grblCore.UIShowUnlockButtons;

	  MnGrbl.Text = _grblCore.Type.ToString();

	  _grblCore.MachineStatusChanged += OnMachineStatus;
	  _grblCore.OnFileLoaded += OnFileLoaded;
	  _grblCore.OnOverrideChange += RefreshOverride;
	  _grblCore.IssueDetected += OnIssueDetected;

	  PreviewForm.SetCore(_grblCore);
	  ConnectionForm.SetCore(_grblCore);
	  JogForm.SetCore(_grblCore);
	  ...
	  if (args != null && args.Length == 1) {
		...
		if (System.IO.File.Exists(filename)) {
		  ...
		  if (System.IO.Path.GetExtension(filename).ToLower() == ".zbn") //zipped button
		  {
			...
			if (_grblCore.CanLoadNewFile)
			  _grblCore.OpenFile(this, filename, false);
			else
	...
	private void ManageMessage() {
	  try {
		...
		_toolBarMessage = UsageStats.Messages.GetMessage(UsageStats.MessageData.MessageTypes.ToolbarLink);
		if (_toolBarMessage != null && _toolBarMessage.Title != null && _toolBarMessage.Content != null) {
		  TTLinkToNews.Text = _toolBarMessage.Title;
	...
	void OnFileLoaded(long elapsed, string filename) {
	  if (InvokeRequired) {
		...
		TTTLines.Text = _grblCore.LoadedFile.Count.ToString();
		//TTTLoadedIn.Text = elapsed.ToString() + " ms";
		TTTEstimated.Text = Tools.Utils.TimeSpanToString(_grblCore.LoadedFile.EstimatedTime, Tools.Utils.TimePrecision.Second, Tools.Utils.TimePrecision.Second, " ,", true);
	  }
	}
	...
	void MainFormFormClosing(object sender, FormClosingEventArgs e) {
	  if (_grblCore.InProgram && System.Windows.Forms.MessageBox.Show(Strings.ExitAnyway, Strings.WarnMessageBoxHeader, MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) != System.Windows.Forms.DialogResult.Yes)
		e.Cancel = true;

	  if (!e.Cancel) {
		SincroStart.StopListen();
		_grblCore.CloseCom(true);
		Settings.SetObject("Mainform Size and Position", new object[] { Size, Location, WindowState });
		Settings.Save();

		UsageStats.SaveFile(_grblCore);
	  }
	}
	...
	private void UpdateTimer_Tick(object sender, EventArgs e) {
	  ...
	  JogForm.Enabled = _grblCore.JogEnabled;
	}

	private void TimerUpdate() {
	  SuspendLayout();
	  TTTStatus.Text = GrblCore.TranslateEnum(_grblCore.MachineStatus);

	  if (_grblCore.InProgram)
		TTTEstimated.Text = Tools.Utils.TimeSpanToString(_grblCore.ProjectedTime, Tools.Utils.TimePrecision.Second, Tools.Utils.TimePrecision.Second, " ,", true);
	  else
		TTTEstimated.Text = Tools.Utils.TimeSpanToString(_grblCore.LoadedFile.EstimatedTime, Tools.Utils.TimePrecision.Second, Tools.Utils.TimePrecision.Second, " ,", true);
	  ...
	  MnFileOpen.Enabled = _grblCore.CanLoadNewFile;
	  MnAdvancedSave.Enabled = MnSaveProgram.Enabled = _grblCore.HasProgram;
	  MnFileSend.Enabled = _grblCore.CanSendFile;
	  MnStartFromPosition.Enabled = _grblCore.CanSendFile;
	  MnRunMulti.Enabled = _grblCore.CanSendFile || _grblCore.CanResumeHold || _grblCore.CanFeedHold;
	  MnGrblConfig.Enabled = true;
	  //MnExportConfig.Enabled = Core.CanImportExport;
	  //MnImportConfig.Enabled = Core.CanImportExport;
	  MnGrblReset.Enabled = _grblCore.CanResetGrbl;
	  ...
	  MnConnect.Visible = !_grblCore.IsConnected;
	  MnDisconnect.Visible = _grblCore.IsConnected;

	  MnGoHome.Visible = _grblCore.Configuration.HomingEnabled;
	  MnGoHome.Enabled = _grblCore.CanDoHoming;
	  MnUnlock.Enabled = _grblCore.CanUnlock;

	  TTOvG0.Visible = _grblCore.SupportOverride;
	  TTOvG1.Visible = _grblCore.SupportOverride;
	  TTOvS.Visible = _grblCore.SupportOverride;
	  spacer.Visible = _grblCore.SupportOverride;
	  ...
	  MnWiFiDiscovery.Enabled = !_grblCore.IsConnected;

	  switch (_grblCore.MachineStatus) {
	  ...
	  PbBuffer.Maximum = _grblCore.BufferSize;
	  PbBuffer.Value = _grblCore.UsedBuffer;
	  PbBuffer.ToolTipText = $"Buffer: {_grblCore.UsedBuffer}/{_grblCore.BufferSize} Free:{_grblCore.FreeBuffer}";
	  MnOrtur.Visible = _grblCore.IsOrturBoard;
	...
	private void RefreshFormTitle() {
	  ...
	  if (_grblCore.Type != Firmware.Grbl)
		FormTitle = FormTitle + $" (for {_grblCore.Type})";
	...
	void RefreshOverride() {
	  SuspendLayout();
	  TTOvG0.Text = string.Format("G0 [{0:0.00}x]", _grblCore.OverrideG0 / 100.0);
	  TTOvG0.BackColor = _grblCore.OverrideG0 > 100 ? Color.LightPink : (_grblCore.OverrideG0 < 100 ? Color.LightBlue : ColorScheme.FormBackColor);
	  TTOvG0.ForeColor = _grblCore.OverrideG0 != 100 ? Color.Black : ColorScheme.FormForeColor;
	  TTOvG1.Text = string.Format("G1 [{0:0.00}x]", _grblCore.OverrideG1 / 100.0);
	  TTOvG1.BackColor = _grblCore.OverrideG1 > 100 ? Color.LightPink : (_grblCore.OverrideG1 < 100 ? Color.LightBlue : ColorScheme.FormBackColor);
	  TTOvG1.ForeColor = _grblCore.OverrideG1 != 100 ? Color.Black : ColorScheme.FormForeColor;
	  TTOvS.Text = string.Format("S [{0:0.00}x]", _grblCore.OverrideS / 100.0);
	  TTOvS.BackColor = _grblCore.OverrideS > 100 ? Color.LightPink : (_grblCore.OverrideS < 100 ? Color.LightBlue : ColorScheme.FormBackColor);
	  TTOvS.ForeColor = _grblCore.OverrideS != 100 ? Color.Black : ColorScheme.FormForeColor;
	...
	internal virtual ContextMenuStrip GetOvMenu() {
	  ...
	  CM.Items.Add(new ToolStripTraceBarItem(_grblCore, 2));
	  CM.Items.Add(new ToolStripTraceBarItem(_grblCore, 1));
	  CM.Items.Add(new ToolStripTraceBarItem(_grblCore, 0));
	  CM.Width = 150;
	  return CM;
	}
	...
	private void MnGoHome_Click(object sender, EventArgs e) {
	  _grblCore.SendHomingCommand();
	}

	private void MnUnlock_Click(object sender, EventArgs e) {
	  _grblCore.SendUnlockCommand();
	}
	...
	private void MnDisconnect_Click(object sender, EventArgs e) {
	  if (!(_grblCore.InProgram && MessageBox.Show(Strings.DisconnectAnyway, Strings.WarnMessageBoxHeader, MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) != System.Windows.Forms.DialogResult.Yes))
		_grblCore.CloseCom(true);
	}

	void MnSaveProgramClick(object sender, EventArgs e) {
	  _grblCore.SaveProgram(this, false, false, false, 1);
	}
	...
	private void SetLanguage(System.Globalization.CultureInfo cultureInfo) {
	  if (cultureInfo != null)
		Settings.SetObject("User Language", cultureInfo);
	  else
	...
	private void settingsToolStripMenuItem_Click(object sender, EventArgs e) {
	  SettingsForm.CreateAndShowDialog(this, _grblCore);
	}
	...
	private void grblConfigurationToolStripMenuItem_Click(object sender, EventArgs e) {
	  GrblConfig.CreateAndShowDialog(this, _grblCore);
	}
	...
	protected override void OnKeyUp(KeyEventArgs e) {
	  ...
	  _grblCore.ManageHotKeys(this, Keys.None);
	  base.OnKeyUp(e);
	}
	...
	protected override bool ProcessCmdKey(ref Message msg, Keys keyData) {
	  if (keyData != mLastkeyData) {
		...
		return _grblCore.ManageHotKeys(this, keyData);
	  } else {
	...
	private void MnReOpenFile_Click(object sender, EventArgs e) {
	  _grblCore.ReOpenFile(this);
	}

	private void fileToolStripMenuItem_DropDownOpening(object sender, EventArgs e) {
	  MnReOpenFile.Enabled = _grblCore.CanReOpenFile;
	}

	private void MnHotkeys_Click(object sender, EventArgs e) {
	  HotkeyManagerForm.CreateAndShowDialog(this, _grblCore);
	}
	...
	private void MnStartFromPosition_Click(object sender, EventArgs e) {
	  _grblCore.RunProgramFromPosition(this);
	}

	private void MnFileAppend_Click(object sender, EventArgs e) {
	  _grblCore.OpenFile(this, null, true);
	}
	...
	private void toolsToolStripMenuItem_DropDownOpening(object sender, EventArgs e) {
	  flashGrblFirmwareToolStripMenuItem.Enabled = (_grblCore.MachineStatus == GrblCore.MacStatus.Disconnected);
	}
	...
	private void activateExtendedLogToolStripMenuItem_Click(object sender, EventArgs e) {
	  if (!ComWrapper.ComLogger.Enabled) {
		using (SaveFileDialog saveFileDialog = new SaveFileDialog()) {
		  ...
		  saveFileDialog.Filter           = "Communication log|*.txt";
		  saveFileDialog.AddExtension     = true;
		  saveFileDialog.OverwritePrompt  = false;
		  saveFileDialog.FileName         = "comlog.txt";
		  saveFileDialog.Title            = "Select extended log filename";
		  ...
		  if (dialogResult == DialogResult.OK && saveFileDialog.FileName != null)
			ComWrapper.ComLogger.StartLog(saveFileDialog.FileName);
		}
	  } else {
	...
	void dropDispatcherTimer_Tick(object sender, EventArgs e) {
	  if (droppedFile != null) {
		_grblCore.OpenFile(this, this.droppedFile);
		droppedFile = null;
	...
	private void MnAdvancedSave_Click(object sender, EventArgs e) {
	  SaveOptionForm.CreateAndShowDialog(this, _grblCore);
	}
	...
	private void TTLinkToNews_Click(object sender, EventArgs e) {
	  if (_toolBarMessage != null && _toolBarMessage.Title != null && _toolBarMessage.Content != null) {
		Tools.Utils.OpenLink(_toolBarMessage.Content);
		UsageStats.Messages.ClearMessage(_toolBarMessage);
	  }
	}
  ...
  public class CustomMenuColor :ProfessionalColorTable {
	public override Color SeparatorDark { get { return ColorScheme.MenuSeparatorColor; } }

	public override Color SeparatorLight { get { return ColorScheme.MenuSeparatorColor; } }
  }
}