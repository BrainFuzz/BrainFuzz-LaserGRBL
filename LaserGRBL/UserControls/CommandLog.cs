...
namespace LaserGRBL.UserControls {
  public partial class CommandLog :UserControl {
	...
	private System.Collections.Generic.List<IGrblRow> mDraw;
	...
	public void SetCom(GrblCore core) { _grblCore = core; }
	...
	protected override void OnPaint(PaintEventArgs e) {
	  ...
	  if (_grblCore != null && e != null && e.Graphics != null) {
		e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
		e.Graphics.TextRenderingHint = System.Drawing.Text.TextRenderingHint.ClearTypeGridFit;

		RowHeight = Font.Height + 2;

		int queueCount = _grblCore.Executed;
		...
		mDraw = _grblCore.SentCommand(index, howmany);
		if (mDraw != null) {
		  using (StringFormat esf = new StringFormat(StringFormat.GenericTypographic)) {
			...
			for (int i = 0; i < mDraw.Count; i++) {
			  ...
			  if (cmd != null) {
				float respectX = 0;
				string result = cmd.GetResult(_grblCore.SupportCSV, mUseImages);
	...
	void SetPosition(int position) {
	  if (mPosition != position) {
		if (mDraw != null && position >= 0 && position < mDraw.Count)
		  TT.SetToolTip(this, Tools.WordWrap.WrapString(mDraw[position].GetToolTip(_grblCore.SupportCSV), 60));
		else
